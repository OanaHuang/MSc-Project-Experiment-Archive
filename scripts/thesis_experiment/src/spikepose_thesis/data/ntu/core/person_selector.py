...
def extract_body_pose_sequence(
    sequence: NTUSkeletonSequence,
    body_id: str,
) -> dict[str, np.ndarray | str]:
    """Extract one explicitly selected tracked person across a sequence."""
    if body_id not in sequence.body_ids:
        raise ValueError(f"Body {body_id!r} is not present in {sequence.path}")
    primary_body_id = body_id
    num_frames = sequence.num_frames
    num_joints = 25
    camera_xyz = np.full((num_frames, num_joints, 3), np.nan, dtype=np.float32)
    depth_xy = np.full((num_frames, num_joints, 2), np.nan, dtype=np.float32)
    color_xy = np.full((num_frames, num_joints, 2), np.nan, dtype=np.float32)
    tracking_state = np.zeros((num_frames, num_joints), dtype=np.int8)
    body_present = np.zeros(num_frames, dtype=bool)
    for frame in sequence.frames:
        body = next((item for item in frame.bodies if item.body_id == body_id), None)
        if body is None:
            continue
        arrays = body.joint_arrays()
        frame_index = frame.frame_index
        camera_xyz[frame_index] = arrays["camera_xyz"]
        depth_xy[frame_index] = arrays["depth_xy"]
        color_xy[frame_index] = arrays["color_xy"]
        tracking_state[frame_index] = arrays["tracking_state"]
        body_present[frame_index] = True
    return {
...
def extract_primary_pose_sequence(
    ...
    return extract_body_pose_sequence(sequence, primary_body_id)
