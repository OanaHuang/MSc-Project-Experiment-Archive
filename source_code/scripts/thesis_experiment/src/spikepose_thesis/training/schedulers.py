...
class WarmupCosineScheduler:
    ...
    def set_epoch(self, epoch: int) -> None:
        ...
        for group in self.optimizer.param_groups:
            group_base = float(group.get("base_lr", self.base_lr))
            group["lr"] = group_base * learning_rate / max(self.base_lr, 1e-12)
...
class ConstantScheduler:
    ...
    def set_epoch(self, epoch: int) -> None:
        self.last_epoch = int(epoch)
        for group in self.optimizer.param_groups:
            group["lr"] = float(group.get("base_lr", self.learning_rate))
