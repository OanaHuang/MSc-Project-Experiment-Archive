...
def crop_and_resize_person(
    ...
    return crop_and_resize_person_with_bbox(
        image=image, keypoints=keypoints, visibility=visibility,
        bbox_xyxy=bbox, output_size=output_size,
    )


def quantize_person_bbox(
    bbox_xyxy: np.ndarray, image_width: int, image_height: int,
) -> np.ndarray:
    """Convert a floating crop box to the exact integer pixel boundaries used by OpenCV."""
    ...
    x1, y1, x2, y2 = bbox
    x1_int = int(np.clip(int(np.floor(x1)), 0, image_width - 1))
    y1_int = int(np.clip(int(np.floor(y1)), 0, image_height - 1))
    x2_int = int(np.clip(int(np.ceil(x2)), x1_int + 1, image_width))
    y2_int = int(np.clip(int(np.ceil(y2)), y1_int + 1, image_height))
    return np.asarray([x1_int, y1_int, x2_int, y2_int], dtype=np.float32)


def crop_and_resize_person_with_bbox(
    image: np.ndarray,
    keypoints: np.ndarray,
    visibility: np.ndarray,
    bbox_xyxy: np.ndarray,
    ...
    """Crop one frame with a caller-provided box shared by a temporal clip."""
    if image.ndim != 3:
        raise ValueError("image must have shape [H, W, C]")
    if output_size <= 0:
        raise ValueError("output_size must be positive")
    keypoints = np.asarray(keypoints, dtype=np.float32).copy()
    visibility = np.asarray(visibility, dtype=np.float32).copy()
    image_height, image_width = image.shape[:2]
    x1_int, y1_int, x2_int, y2_int = (
        int(value) for value in quantize_person_bbox(
